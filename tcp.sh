...
sh_ver="1.3.2"
github="raw.githubusercontent.com/huiaini/Linux-NetSpeed/master"
...
startbbrmod_nanqinlang(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/huiaini/Linux-NetSpeed/master/bbr/tcp_nanqinlang.c
		echo "obj-m := tcp_nanqinlang.o" > Makefile
		make -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=/usr/bin/gcc-4.9
...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/huiaini/lotServer/master/Install.sh) install
